document.addEventListener('DOMContentLoaded', () => {
    ...
    // ===================================================================
    const GOOGLE_SCRIPT_URL = 'https://script.google.com/macros/s/AKfycbxteUKL7pmD6QyZUZJpVCECWQOiHCEMMn7k14J3LsD1-kzmi0NcnfAlUjdesjRFW5WV4Q/exec';
    ...
    window.addEventListener('click', (e) => {
        if (e.target == loginModal || e.target == panelModal) closeModal();
    });
    ...
    async function fetchUnpaidUsers() {
        ...
        try {
            // MODIFIÉ : Ajout du cache buster
            const url = `${GOOGLE_SCRIPT_URL}?action=getUnpaid&_cacheBuster=${new Date().getTime()}`;
    ...
    async function handleMarkAsPaid(event) {
        ...
        try {
            // MODIFIÉ : Ajout du cache buster
            const url = `${GOOGLE_SCRIPT_URL}?action=markAsPaid&id=${id}&_cacheBuster=${new Date().getTime()}`;
            const response = await fetch(url);
            const result = await response.json();
            if (!result.success) throw new Error(result.error);
            fetchUnpaidUsers();
    ...
    async function verifyTicket(id) {
        try {
            // MODIFIÉ : Ajout du cache buster
            const url = `${GOOGLE_SCRIPT_URL}?action=checkin&id=${id}&_cacheBuster=${new Date().getTime()}`;
            const response = await fetch(url);
            const result = await response.json();
            if (!result.success) throw new Error(result.error);
